...
The primary AI-driven workflow resides within the system's backend processing core, designed as a service-oriented architecture. This backend, built with FastAPI, orchestrates a pipeline for \textbf{real-time analysis of live video streams}. Video data allows for immediate ingestion from IP cameras (e.g., via RTSP) and processed in-memory for low-latency analysis. The AI pipeline involves several key steps for each frame extracted from these real-time feeds:
\begin{enumerate}
    \item \textbf{Person Detection:} Identifying individuals within each camera frame using high-performance detection models (e.g., YOLOv26m).
    ...
    \item \textbf{Space-Based Matching (Continuous):} For every tracked individual, transforming 2D pixel coordinates to a unified 3D map location using homography. This "always-on" step handles overlapping views by geometrically predicting location, allowing the system to bridge tracks spatially before resorting to visual features.
    \item \textbf{Feature Extraction:} Generating unique appearance embeddings (features) from detected person images using models like Torchreid/OSNet-AIN, preparing them for potential identification needs.
    \item \textbf{Cross-Camera Re-Identification (Conditional):} Triggered only when a target leaves one camera view and enters another (non-overlapping) view. This step matches appearance embeddings against the gallery to confirm identity when spatial continuity alone is insufficient.
    \item \textbf{Spatial Mapping \& Visualization:} The final 2D map output which visualizes the consolidated tracks, derived from the continuous Space-Based Matching data.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Processing Approach \& Model Selection (ML Canvas):}
        ...
        \begin{enumerate}[label=(\Alph*)]
            \item Person Detection, utilizing YOLOv26m (Real-Time DEtection TRansformer) for robust object localization.
            ...
            \item Feature Extraction for Re-ID, using Torchreid/OSNet-AIN to generate discriminative embeddings.
...
Figure \ref{fig:ml_pipeline_stages} visually summarizes the core AI processing stages detailed in the "Processing Approach \& Model Selection (ML Canvas)" item. It outlines the sequential data flow from Ingestion and Object Detection through Intra-Camera Tracking, incorporating a conditional Zone Crossing decision point that routes processing to either Re-ID or Spatial Mapping, before concluding with Broadcast \& Persistence.

\begin{figure}[!htb]
    \centering
    \makebox[\textwidth][c]{
        \includegraphics[width=1.0\textwidth, keepaspectratio]{jubjones/dataflow_diagram.jpg} 
    }
    \caption{Core AI Processing Data Flow. This diagram illustrates the sequence from Ingestion and Object Detection to Intra-Camera Tracking, highlighting the conditional Zone Crossing decision that directs the flow toward either Re-ID or Spatial Mapping before culminating in Broadcast \& Persistence.}
...
The primary AI model for person detection is YOLOv26m (Real-Time Detection Transformer), developed using the PyTorch deep learning framework. The development process prioritized robustness and accuracy for surveillance footage analysis.
...
\begin{itemize}
    ...
    \item \textbf{Fine-Tuning Approach:} The YOLOv26m model was initialized with default weights and fine-tuned on the MTMMC dataset. The model was trained using an NVIDIA RTX 4060 GPU to specifically address the 'person' class detection relevant to the system's objectives.
...
\begin{itemize}
    ...
    \item \textbf{Comparative Model Analysis:} To select the most suitable baseline detection model for the system, a comparative analysis of different candidate architectures was performed. This involved training and evaluating several models (e.g., Faster R-CNN, YOLOv26m variants) under consistent conditions. Performance was assessed based on quantitative accuracy metrics (like mAP), inference speed (Frames Per Second - FPS), and qualitative observations of detection stability on representative video data. The process of comparing models, including visualization of performance trade-offs (as conceptually illustrated in Figure \ref{fig:mlflow_comparison_1}) and tabular summaries of key metrics (conceptually represented by Figure \ref{fig:mlflow_comparison_2}), guided the selection.
...
The comparative analysis of various person detection models, aided by experiment tracking tools, led to the selection of YOLOv26m as the baseline architecture for the \usevar{\srsTitle} system. After evaluating candidates including Faster R-CNN and YOLOv26m variants, YOLOv26m presented a compelling balance of characteristics crucial for the system's application in retrospective surveillance analysis.

Key performance observations and the justification for selecting YOLOv26m include:
\begin{itemize}
    \item \textbf{End-to-End Detection:} YOLOv26m provides end-to-end detection without requiring Non-Maximal Suppression (NMS) post-processing, reducing the complexity of the detection pipeline and eliminating NMS-related artifacts.
    \item \textbf{Strong Overall Accuracy:} YOLOv26m demonstrated competitive mAP scores on the MTMMC validation set, particularly for medium to large-sized persons common in surveillance footage.
...
To build trust and enable diagnostics, techniques were employed to understand and explain the behavior of the selected person detection model (YOLOv26m).
...
Explainability is a critical factor for trust and adoption in AI-driven surveillance systems. In SpotOn, techniques such as Grad-CAM visualization and interpretable performance metrics provide insight into model behavior and decision-making processes. The team will systematically \textbf{communicate evaluation, quality, and risk results} to advisors, ensuring all architectural choices are \textbf{evidence-based decisions}. These mechanisms enable operators and system maintainers to understand, validate, and diagnose AI outputs, thereby increasing confidence in automated identity association and supporting responsible real-world deployment.
\begin{figure}[!htb]
    \centering
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=1.0\textwidth,keepaspectratio]{assets/jubjones/UI_ETC/map.png}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=1.0\textwidth,keepaspectratio]{assets/jubjones/UI/DETECTED_PEOPLE.jpeg}
...
\begin{itemize}
    ...
    \item \textbf{AI Communication with System Components:}
        ...
        \begin{itemize}
            ...
            \item \textbf{Backend to Frontend (Real-time Video \& Metadata):}
                \begin{itemize}
                    \item \textbf{MJPEG Streaming:} The backend streams raw video frames directly to the frontend using the \textbf{MJPEG} protocol. This ensures low-latency video playback without requiring intermediate cloud storage for live feeds.
                    \item \textbf{WebSocket:} Processed tracking metadata (bounding boxes, local IDs, global IDs from space-based and Re-ID matching, map coordinates, frame timestamps) are pushed from the FastAPI Backend to the React Frontend using \textbf{WebSocket} connections. This enables real-time updates for client-side visualization overlays on top of the MJPEG video stream.
                \end{itemize}
            \item \textbf{Frontend to Backend (Control/Queries):} User interactions (e.g., initiating analysis tasks, requesting historical data, system configuration) are sent from the React Frontend to the FastAPI Backend via \textbf{RESTful APIs}. External communication passes through a reverse proxy like Nginx.
        \end{itemize}
    ...
    \item \textbf{Tools and Frameworks Used:}
        ...
        \begin{itemize}
            ...
            \item \textit{AI Models \& Libraries:} \textbf{PyTorch} as the deep learning framework. Specific models like \textbf{YOLOv26m} (detection), \textbf{ByteTrack} (tracking), \textbf{Torchreid/OSNet-AIN} (Re-ID feature extraction). Space-based location prediction uses homography transformations. \textbf{OpenCV} for video processing and image manipulation.
    ...
    \item \textbf{System Qualities (Reliability, Security, Maintainability, Scalability):}
        ...
        \end{itemize}
    \item \textbf{MLOps \& Governance:}
        \begin{itemize}
            \item \textbf{Observability:} The system includes production-grade \textbf{observability and monitoring} to track model accuracy and concept drift.
            \item \textbf{Risk Management:} Deployment includes \textbf{guardrails} for low-confidence detections and \textbf{incident playbooks} for handling tracking failures.
        \end{itemize}
\end{itemize}
