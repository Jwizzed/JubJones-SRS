...
\begin{enumerate}[leftmargin=80pt]
    \item \textbf{Multi-Stage AI Pipeline:} The backend integrates YOLOv26m for high-accuracy person detection, ByteTrack for intra-camera temporal identity preservation, and a Torchreid-based Re-Identification module for cross-camera identity resolution. Together, these components form a coherent and ordered processing pipeline that transforms raw video frames into structured tracking metadata.
...
All 38 frontend test cases and 50 backend test cases were verified and passed, confirming the system's functional completeness and reliability. The system was rigorously evaluated on the MTMMC dataset across a university campus and an industrial factory environment. The full SpotOn pipeline achieved an average Multiple Object Tracking Accuracy (MOTA) of 54.3\%, an IDF1 score of 61.8\%, and an end-to-end per-frame processing latency of 49.4 ms across four concurrent camera streams. Crucially, the system demonstrated that prioritizing homography-based spatial matching for overlapping camera views---while reserving appearance-based Re-ID exclusively for 5\% camera-edge zones---dramatically reduced computational overhead without sacrificing identity continuity.
