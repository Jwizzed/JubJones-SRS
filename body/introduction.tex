...
This is a \textbf{cross-disciplinary project} that actively \textbf{aligns users, engineering, and security policy}. Our project aims to develop a robust tracking system called \textit{SpotOn} that consistently identifies and tracks individuals across camera networks, providing uninterrupted coverage in campus and factory settings. The project methodology relies on designing solutions systematically through a \textbf{hypothesis $\rightarrow$ test $\rightarrow$ iterate} cycle. By solving the identity maintenance challenge across camera transitions, our solution will enhance security monitoring, expedite emergency responses, and provide valuable data for facility management and resource optimization.
...
\begin{enumerate}[leftmargin=80pt]
    \item \textbf{Agentic LLM Interface with Tool-Calling:}
    \begin{itemize} [topsep=0pt, partopsep=0pt, itemsep=0pt, parsep=0pt] 
        \item The LLM utilizes RAG (Retrieval-Augmented Generation) to fetch real-time tracked person metadata and uses tool-calling to autonomously trigger camera switches or Re-ID queries based on user prompts.
        \item Implemented LLMOps practices, including prompt guardrails to prevent system hallucinations, and integrated observability/monitoring for the LLM agent.
        \item The backend uses dynamic routing and model selection to choose between lightweight and heavy LLM models based on the complexity of the user's query.
    \end{itemize}
...
\begin{table}[p]
    ...
    \noindent\begin{tabular}{| p{2.65in} | p{2.65in} |}
        \hline & \\[-10pt] % Top rule adjustment space
        % --- Row 1 ---
        \persona{Security Officer}
        {Pursue persons of interest (POIs) seamlessly, detect breaches.} % Preferences
        {Broken suspect trails during handoffs, slow incident reaction.} % Pain Points
        {Continuous POI monitoring, faster threat neutralization, secure evidence.} % Goals
         &
        \persona{Facility Manager}
        {Analyze movement flow, measure area usage accurately.} % Preferences
        {Incomplete traffic data, bottlenecks unidentified.} % Pain Points
        {Optimize spatial layout, understand usage trends, enhance workflow.} % Goals
         \\[10pt] % Space between rows
        \hline & \\[-10pt] % Mid rule adjustment space
        % --- Row 2 ---
        % Persona 3 in the first column
        \persona{Emergency Coordinator}
        {Oversee evacuations clearly, assess crowd levels instantly.} % Preferences
        {Gaps in crisis zone visibility, delayed finding of individuals.} % Pain Points
        {Guide safe evacuations, speed up assistance delivery, maintain situational control.} % Goals
         &
        % Second column is intentionally left empty to maintain the 2x2 structure visually
         \\[10pt] % Space below the last persona row
        \hline % Bottom rule
    \end{tabular}
...
\subsection{Personal Development Plan}
\label{subsection:personal-development-plan}

The team has designed a \textbf{competency-based learning pathway} linked to this project, which serves as a core portfolio output. To validate our skills in Computer Vision, we will run \textbf{small experiments and mini-projects} comparing different Re-ID toolboxes (like FastReID vs. YoloRe-IDNet). We will use \textbf{feedback and metrics (such as rubric scores and completion rates)} to continuously adapt our learning, demonstrating \textbf{fast learning and skill transfer} from academic theory to applied surveillance contexts.
