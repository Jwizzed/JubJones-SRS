...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{DetectionVideoService:} The central orchestrator that manages video stream processing, invoking object detectors (YOLO26m) and trackers.
...
%     participant Detector as Object Detector (YOLO26m)
...
2.  Frames are passed to the \textbf{Object Detector} (YOLO26m) to identify individuals.
...
\begin{enumerate}[leftmargin=80pt]
    ...
    \item \textbf{Object Detection:} The YOLO26m model analyzes each frame to output bounding boxes and confidence scores for all detected persons.
...
\begin{itemize}
    \item \textbf{Detection Model:} YOLO26m (Real-Time Detection Transformer) - Selected for its balance of speed and accuracy, specifically its ability to handle crowded scenes better than traditional YOLO-based approaches. This model is responsible for identifying "person" objects within each camera frame.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item YOLO26m: Fine-tuned on MTMMC dataset
...
\subsubsection{System and API Health Metrics}
\label{subsubsection:system-api-metrics}

\begin{itemize}
    \item \textbf{API Response Time (p50, p95, p99):} Average and peak times required for REST endpoints to return data, ensuring responsiveness for historical and analytical queries
    \item \textbf{Throughput (Requests Per Second - RPS):} The volume of concurrent requests the API and WebSocket services can handle before performance degrades
    \item \textbf{Error Rate:} The percentage of failed HTTP requests (4xx, 5xx) and dropped WebSocket connections over time
\end{itemize}

\subsubsection{Real-Time Processing Performance}
\label{subsubsection:real-time-performance}

\begin{itemize}
    \item \textbf{Glass-to-Glass Latency:} The total time delay from an event occurring in the physical world to its visualization on the user's dashboard with AI overlays
    \item \textbf{Pipeline Processing Latency:} The duration for the backend to ingest a video frame, execute detection and tracking pipelines, and emit the resulting metadata
    \item \textbf{Stream Initialization Time:} The time elapsed between a user requesting a camera feed and the first video frame rendering on the interface
    \item \textbf{WebSocket Message Latency:} The delivery delay of tracking coordinate updates from backend emission to frontend reception
    \item \textbf{Frame Drop Rate:} The percentage of video frames dropped due to backend processing overload or frontend rendering constraints
\end{itemize}

\subsubsection{Frontend User Experience Metrics}
\label{subsubsection:frontend-ux-metrics}

\begin{itemize}
    \item \textbf{Time to First Byte (TTFB):} The delay between the browser's request and receiving the first byte of data from the server
    \item \textbf{Largest Contentful Paint (LCP):} The render time of the largest visual element, primarily the video player or the spatial map
    \item \textbf{Time to Interactive (TTI):} The time taken for the dashboard to become fully responsive and capable of handling user interactions reliably
    \item \textbf{UI Frame Rate (FPS):} The rendering speed of the client application, ensuring smooth visualization of moving bounding boxes and spatial markers
\end{itemize}
...
The AI component is a critical element of our system, enabling sophisticated \textbf{real-time} processing of video feeds for object detection, tracking, space-based location prediction, and re-identification. By leveraging state-of-the-art models like yolo26m, ByteTrack, and FastReID, trained on the comprehensive MTMMC dataset, our system achieves high accuracy even in challenging scenarios. The training methodology using NVIDIA RTX 4060 GPU, combined with continuous learning approaches and ethical considerations, ensures that the AI component remains effective and responsible. The space-based location prediction algorithm enables intra-frame global ID assignment for overlapping camera views, complementing the Re-ID model for across-frame matching. The seamless integration with other system components via MJPEG, WebSocket, and REST APIs provides a scalable and reliable real-time solution for our users.